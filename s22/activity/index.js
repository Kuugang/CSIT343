...
let adventurer = {
  ...
  attackMonster: function (monster) {
    if (monster.health <= 0) {
      return `${monster.name} is already dead. That's overkill!`;
    }
    monster.health -= this.attack;
    if (monster.health <= 0) {
      monster.dropLoot(this);
      return `${this.name} attacks ${monster.name}!, ${monster.name} has been defeated!, ${monster.name} dropped ${monster.loot}`;
    }
...
function Monster(name, level, health, attack, loot) {
  ...
  this.attackTarget = function (character) {
    ...
    return `${this.name} attacks ${character.name}!, ${character.name}'s is now ${character.health}`;
  };
  // Method for the monster to drop loot when defeated
  this.dropLoot = function (character) {
    character.inventory.push(this.loot);
    return `${this.name} drops ${this.loot}. ${character}'s inventory ${character.inventory}`;
  };
...
let fenrir = new Monster("Fenrir the Wolf", 30, 100, 10, "Wolf Fur");
